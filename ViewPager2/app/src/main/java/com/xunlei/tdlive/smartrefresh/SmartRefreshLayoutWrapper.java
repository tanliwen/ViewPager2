...
public class SmartRefreshLayoutWrapper extends ViewGroup implements RefreshLayout, NestedScrollingParent/*, NestedScrollingChild*/ {
...
//                    @Override
//                    public void run() {
//                        final View thisView = SmartRefreshLayoutWrapper.this;
//                        for (ViewParent parent = thisView.getParent() ; parent != null ; ) {
//                            if (parent instanceof NestedScrollingParent) {
//                                View target = SmartRefreshLayoutWrapper.this;
        ...
        if (Math.abs(velocity) > mMinimumVelocity) {
            if (velocity * mSpinner < 0) {
                ...
                if (mState == RefreshState.Refreshing || mState == RefreshState.Loading || (mSpinner < 0 && mFooterNoMoreData)) {
                    animationRunnable = new SmartRefreshLayoutWrapper.FlingRunnable(velocity).start();
                    return true;
        ...
        AnimatorListenerAdapter listener = new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                if (mRefreshListener != null) {
                    if (notify) {
                        mRefreshListener.onRefresh(SmartRefreshLayoutWrapper.this);
                    }
                } else if (mOnMultiPurposeListener == null) {
                    finishRefresh(3000);
                }
                if (mRefreshHeader != null) {
                    mRefreshHeader.onStartAnimator(SmartRefreshLayoutWrapper.this, mHeaderHeight, (int) (mHeaderMaxDragRate * mHeaderHeight));
                }
                if (mOnMultiPurposeListener != null && mRefreshHeader instanceof RefreshHeader) {
                    if (notify) {
                        mOnMultiPurposeListener.onRefresh(SmartRefreshLayoutWrapper.this);
                    }
    ...
    protected void moveSpinnerInfinitely(float spinner) {
        ...
        if (mEnableAutoLoadMore && !mFooterNoMoreData && isEnableRefreshOrLoadMore(mEnableLoadMore) && spinner < 0
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    if (mLoadMoreListener != null) {
                        mLoadMoreListener.onLoadMore(SmartRefreshLayoutWrapper.this);
                    } else if (mOnMultiPurposeListener == null) {
                        finishLoadMore(2000);//如果没有任何加载监听器，两秒之后自动关闭
                    }
                    final OnLoadMoreListener listener = mOnMultiPurposeListener;
                    if (listener != null) {
                        listener.onLoadMore(SmartRefreshLayoutWrapper.this);
                    }
                }
            }, mReboundDuration);
        }
...
//    }
    ...
    public RefreshLayout setRefreshHeader(@NonNull RefreshHeader header, int width, int height) {
        ...
        if (mRefreshHeader.getSpinnerStyle().front) {
            ...
            super.addView(mRefreshHeader.getView(), thisGroup.getChildCount(), new com.scwang.smartrefresh.layout.SmartRefreshLayout.LayoutParams(width, height));
        } else {
            super.addView(mRefreshHeader.getView(), 0, new com.scwang.smartrefresh.layout.SmartRefreshLayout.LayoutParams(width, height));
        }
    ...
    public RefreshLayout setRefreshFooter(@NonNull RefreshFooter footer, int width, int height) {
        ...
        if (mRefreshFooter.getSpinnerStyle().front) {
            final ViewGroup thisGroup = this;
            super.addView(mRefreshFooter.getView(), thisGroup.getChildCount(), new com.scwang.smartrefresh.layout.SmartRefreshLayout.LayoutParams(width, height));
        } else {
            super.addView(mRefreshFooter.getView(), 0, new com.scwang.smartrefresh.layout.SmartRefreshLayout.LayoutParams(width, height));
        }
    ...
    public RefreshLayout setRefreshContent(@NonNull View content, int width, int height) {
        ...
        final ViewGroup thisGroup = this;
        super.addView(content, thisGroup.getChildCount(), new com.scwang.smartrefresh.layout.SmartRefreshLayout.LayoutParams(width, height));
...
//                        resetStatus();
                    ...
                    int startDelay = mRefreshHeader.onFinish(SmartRefreshLayoutWrapper.this, success);
                    if (mOnMultiPurposeListener != null && mRefreshHeader instanceof RefreshHeader) {
                    ...
                    if (startDelay < Integer.MAX_VALUE) {
                        ...
                        if (mIsBeingDragged || mNestedInProgress) {
                            ...
                            if (mIsBeingDragged) {
                                ...
                                mTouchSpinner = 0;
                                mIsBeingDragged = false;
                                SmartRefreshLayoutWrapper.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_DOWN, mLastTouchX, mLastTouchY + mSpinner - mTouchSlop * 2, 0));
                                SmartRefreshLayoutWrapper.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_MOVE, mLastTouchX, mLastTouchY + mSpinner, 0));
                            }
                            if (mNestedInProgress) {
                                mTotalUnconsumed = 0;
                                SmartRefreshLayoutWrapper.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_UP, mLastTouchX, mLastTouchY, 0));
                                mNestedInProgress = false;
                                mTouchSpinner = 0;
                            }
                        }
...
//                       resetStatus();
                    ...
                    final int startDelay = mRefreshFooter.onFinish(SmartRefreshLayoutWrapper.this, success);
                    if (mOnMultiPurposeListener != null && mRefreshFooter instanceof RefreshFooter) {
                    ...
                    if (startDelay < Integer.MAX_VALUE) {
                        ...
                        if (mIsBeingDragged || mNestedInProgress) {
                            ...
                            if (mIsBeingDragged) {
                                ...
                                SmartRefreshLayoutWrapper.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_DOWN, mLastTouchX, mLastTouchY + offsetY + mTouchSlop * 2, 0));
                                SmartRefreshLayoutWrapper.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_MOVE, mLastTouchX, mLastTouchY + offsetY, 0));
                            }
                            if (mNestedInProgress) {
                                mTotalUnconsumed = 0;
                                SmartRefreshLayoutWrapper.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_UP, mLastTouchX, mLastTouchY, 0));
...
//    }
    ...
    public class RefreshKernelImpl implements RefreshKernel {
        ...
        public RefreshLayout getRefreshLayout() {
            return SmartRefreshLayoutWrapper.this;
        }
...
//                            mRefreshFooter.getView().requestLayout();
        ...
        public ValueAnimator animSpinner(int endSpinner) {
            return SmartRefreshLayoutWrapper.this.animSpinner(endSpinner, 0, mReboundInterpolator, mReboundDuration);
        }
