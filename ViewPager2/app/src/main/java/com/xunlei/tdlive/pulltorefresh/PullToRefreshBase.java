...
import com.xunlei.tdlive.pulltorefresh.internal.RotateLoadingLayout;
import com.xunlei.tdlive.pulltorefresh.internal.Utils;
import com.xunlei.tdlive.pulltorefresh.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    static final float FRICTION = 2.0f;//default 2.0
    ...
    private Mode mCurrentMode;   // 指示：当前是向上拉动  还是向下拉动
    ...
    protected  boolean canForceHeaderScroll(){
        if((mState == State.REFRESHING&&mCurrentMode== Mode.PULL_FROM_START)||mState == State.MANUAL_REFRESHING){
            return false;
        }
        return true;
    }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (mIsBeingDragged) {
                    ...
                    // 刷新中，再次滑动屏幕 ， just scroll back to the top
                    if (isRefreshing()) {
                        if (needResetToHeadBegin()){ //
    ...
    public final void setMode(Mode mode) {
        if (mode != mMode) {
            if (DEBUG) {
                android.util.Log.d(LOG_TAG, "Setting mode to: " + mode);
            }
            mMode = mode;
            updateUIForMode();
        }
    ...
    public void setOnPullScrollListener(OnPullScrollListener listener){
        mOnPullScrollListener = listener;
    }
    ...
    public void setScrollAnimationInterpolator(android.view.animation.Interpolator interpolator) {
    ...
    protected final int getHeaderSize() {
        if(mHeaderHeight == 0){
            mHeaderHeight = mHeaderLayout.getContentSize();
        }
     ...
     */
    protected void onPtrSaveInstanceState(android.os.Bundle saveState) {
    }
    ...
    protected final android.os.Parcelable onSaveInstanceState() {
        android.os.Bundle bundle = new android.os.Bundle(9);
    ...
    protected final void setScrollValue(int value) {
        // Clamp value to with pull scroll range
        final int maximumPullScroll = getMaximumPullScroll();
        value = Math.min(maximumPullScroll, Math.max(-maximumPullScroll, value));  //保证 -maxnumPulllScroll <= value <= maxinumPullScroll

        if (mLayoutVisibilityChangesEnabled) {
            if (value < 0) {
                mHeaderLayout.setVisibility(View.VISIBLE);
                mHeaderLayout.onHeaderScroll(value,mState,mCurrentMode);
            } else if (value > 0) {
                mFooterLayout.setVisibility(View.VISIBLE);
                mFooterLayout.onHeaderScroll(value,mState,mCurrentMode);
            } else {
        ...
        if(mState!= State.MANUAL_REFRESHING){
            if(getMode() == Mode.PULL_FROM_START && !mIsPressing && value <0 &&  value < mLastValue){
                return;
            }
        }
        ...
        if(mOnPullScrollListener != null ){
            mOnPullScrollListener.onPullScroll(value,mLastValue);
        }
    ...
    boolean canUseLayer(){
        return VERSION.SDK_INT >= 11 && USE_HW_LAYERS;
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        // Styleables from XML
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.PullRefresh,0,0);

        if (a.hasValue(R.styleable.PullRefresh_prMode)) {
            mMode = Mode.mapIntToValue(a.getInteger(R.styleable.PullRefresh_prMode, 0));
        }

        if (a.hasValue(R.styleable.PullRefresh_prAnimationStyle)) {
            mLoadingAnimationStyle = AnimationStyle.mapIntToValue(a.getInteger(
                    R.styleable.PullRefresh_prAnimationStyle, 0));
        }
         ...
         * Styleables from XML
         */
        if (a.hasValue(R.styleable.PullRefresh_prRefreshableViewBackground)) {
            android.graphics.drawable.Drawable background = a.getDrawable(R.styleable.PullRefresh_prRefreshableViewBackground);
            if (null != background) {
                mRefreshableView.setBackgroundDrawable(background);
            }
        } else if (a.hasValue(R.styleable.PullRefresh_prAdapterViewBackground)) {
            Utils.warnDeprecation("ptrAdapterViewBackground", "ptrRefreshableViewBackground");
            android.graphics.drawable.Drawable background = a.getDrawable(R.styleable.PullRefresh_prAdapterViewBackground);
            if (null != background) {
                mRefreshableView.setBackgroundDrawable(background);
            }
        }

        if (a.hasValue(R.styleable.PullRefresh_prOverScroll)) {
            mOverScrollEnabled = a.getBoolean(R.styleable.PullRefresh_prOverScroll, true);
        }

        if (a.hasValue(R.styleable.PullRefresh_prScrollingWhileRefreshingEnabled)) {
            mScrollingWhileRefreshingEnabled = a.getBoolean(
                    R.styleable.PullRefresh_prScrollingWhileRefreshingEnabled, false);
        }

        if(a.hasValue(R.styleable.PullRefresh_prFooterRefreshViewShow)){
            boolean show = a.getBoolean(R.styleable.PullRefresh_prFooterRefreshViewShow, true);
            if(!show){
                setFooterViewNotShow();
            }
        }

        if(a.hasValue(R.styleable.PullRefresh_prHeaderRefreshViewShow)){
            boolean show = a.getBoolean(R.styleable.PullRefresh_prHeaderRefreshViewShow, true);
            if(!show){
    ...
    private void pullEvent() {
        ...
        switch (mCurrentMode) {
            ...
            default:
//				if(isRefreshing()){
//					newScrollValue = Math.round(Math.min(initialMotionValue - lastMotionValue, 0) / FRICTION) - getHeaderSize();
//				}else{
...
//				}
    ...
    private final void smoothScrollTo(int newScrollValue, long duration, long delayMillis,
        ...
        if (oldScrollValue != newScrollValue) {
            if (null == mScrollAnimationInterpolator) {
                ...
                mScrollAnimationInterpolator = new android.view.animation.DecelerateInterpolator();
            }
    ...
    public static enum Mode {
         ...
         */
        boolean permitsPullToRefresh() {//修改 by 王鹏
    ...
    public interface OnPullScrollListener{
        void onPullScroll(int scrollValue, int lastValue);
    }
    ...
    public void setScrollBar(int resId){}
